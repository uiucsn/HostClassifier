...
def customLoss(yTrue, yPred):
    ...
    """
    ## A. Gagliano modified to be of length 6
    return K.mean(K.square(yTrue[:, :, 0:6] - yPred[:, :, :]))
...
def make_model(LSTMN, encodingN, maxlen, nfilts):
    ...
    repeater = RepeatVector(int(maxlen))(encoded)
...
def get_decodings(decoder, encoder, sequence, lms, encodingN, sequence_len, plot=True):
    ...
    if plot:
        for i in np.arange(len(sequence)):
            ...
            seq = np.reshape(sequence[i, :, :], (1, sequence_len, 13))
