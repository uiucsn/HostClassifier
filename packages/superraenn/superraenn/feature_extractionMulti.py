...
def feat_rise_and_decline(input_lcs, n_mag, nfilts=6):
...
def feat_slope(input_lcs, t_min_lim=10, t_max_lim=30, nfilts=6):
    slopes_all = []
    for i, input_lc in enumerate(input_lcs):
        gp = input_lc.gp
        gp_mags = input_lc.gp_mags
...
def feat_int(input_lcs, nfilts=6):
    ints_all = []
    for i, input_lc in enumerate(input_lcs):
        gp = input_lc.gp
        gp_mags = input_lc.gp_mags
...
def main():
    ...
    if args.get_feat_rise_decline1:
      #  feat1, feat2 = feat_rise_and_decline(input_lcs, 1)
        feat1, feat2 = pool.starmap(feat_rise_and_decline, [(row, 1) for row in input_lcs])
        if features != []:
...
#        feat1, feat2 = feat_rise_and_decline(input_lcs, 2)
        feat1, feat2 = pool.starmap(feat_rise_and_decline, [(row, 2) for row in input_lcs])
        if features != []:
            features = np.hstack((features, feat1))
            features = np.hstack((features, feat2))
        else:
...
#        feat1, feat2 = feat_rise_and_decline(input_lcs, 3)
        feat1, feat2 = pool.starmap(feat_rise_and_decline, [(row, 3) for row in input_lcs])
        if features != []:
            features = np.hstack((features, feat1))
            features = np.hstack((features, feat2))
        else:
...
#        feat = feat_slope(input_lcs)
        feat = pool.starmap(feat_slope, [row for row in input_lcs])
        if features != []:
            features = np.hstack((features, feat))
        else:
            features = feat
