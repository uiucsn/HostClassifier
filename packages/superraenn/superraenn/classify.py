...
def prep_data_for_classifying(featurefile, means, stds, whiten=True, verbose=False):
    ...
    #read in host-galaxy features!
    
    hostpath = "/Users/alexgagliano/Documents/Research/TransientRecommender/superraenn/products/host_info_20k.tar.gz"
    hostInfo = pd.read_csv(hostpath)

    #subset by the values actually in the dataset
    #try to preserve the original order 
    ids_int = np.array([int(x) for x in ids])
    hostInfo = hostInfo.set_index('ID').loc[ids_int].reset_index()
    hostSFR = hostInfo['SFR'].values
    hostMass = hostInfo['Msol'].values

    meanSFR = np.nanmean(hostSFR)
    meanMass = np.nanmean(hostMass) 

    stdSFR = np.nanstd(hostSFR)
    stdMass = np.nanstd(hostMass)

    #add mean and std
    #means = np.concatenate([means, [meanSFR], [meanMass]])
    #stds = np.concatenate([stds, [stdSFR], [stdMass]])

    features = np.column_stack((features, hostSFR, hostMass))
    feat_names = np.concatenate([feat_names, ['host_logSFR'], ['host_logMsol']])
    #note -- means and stds already done in training! Just to make sure: 
    print(feat_names)
    ...
    if args.train:
        ...
        if not args.savemodel:
            ...
            for train_index, test_index in loo.split(X):
                ...
                #A. Gagliano removing all params in case they're optimized
                ...
                if args.calc_importance:
                    ...
                    plt.savefig("superRAENN_derivedHost_FeatureImportance.png",dpi=200, bbox_inches='tight')
                    plt.show()
